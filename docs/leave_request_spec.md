...
社員は、自分の申請中の有給申請を取り消せる。

- 取消できるのは `pending` の申請のみ
- 承認済み・却下済みの申請は取消不可
- 取消時は `status` を `cancelled` に更新し、`cancelledBy` / `cancelledAt` を保存する
- 取消時は有給残日数を変更しない（申請時点で残日数を減らしていないため）
- 取消済み申請は履歴として残り、管理者も確認できる
...
- 出力場所：管理者ダッシュボード「CSV出力」セクション、有給申請一覧画面
- 出力対象：申請中・承認済み・却下済みのすべての申請履歴
- ステータスは日本語で出力（申請中 / 承認済み / 却下 / 取消）
- BOM付きUTF-8でExcelでも日本語が文字化けしない

詳細は `docs/csv_export_spec.md` を参照。
