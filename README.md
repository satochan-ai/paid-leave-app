...
管理者向けに以下のCSV出力に対応しました。

（中略）

### Round 9：有給付与チェック

入社日と現在日をもとに、付与されているべき有給休暇を確認し、不足分を自動生成できるようにしました。

- 全社員の付与チェック（管理者ダッシュボード）
- 社員別の付与チェック（社員詳細画面）
- 重複付与防止
- 退職者は対象外
...
管理者向けにlocalStorageデータのJSONバックアップ・復元に対応しました。
...
ログイン中ユーザー情報 currentUser はバックアップ対象外です。

#### 注意

復元時は現在のデータが上書きされます。
復元後はログアウト状態になり、再ログインが必要です。
...
CSVはExcelで開きやすいようにBOM付きUTF-8で出力します。
