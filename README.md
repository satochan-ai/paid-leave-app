...
- 管理者ログイン
- 管理者ダッシュボード（KPI7種・失効予定アラート・クイックアクション・未承認申請数）
- 社員一覧（検索・ステータス絞り込み・付与区分表示）
- 社員追加・編集（通常付与/比例付与の勤務条件設定）
- 社員詳細（仮ログイン情報確認・有給サマリー・付与/取得履歴）
- 有給取得登録（1日・半休0.5日）
- 有給申請一覧（申請確認・承認・却下・取消済み確認・ステータスフィルター）
...
- 社員ログイン
- マイページ（有給残日数・勤務条件・次回付与日・直近失効日）
- 有給申請（取得希望日・取得日数・理由を入力・残日数不足チェック）
- 申請履歴確認（ステータス・却下理由）
- 申請中の有給申請取消
- 付与履歴・取得履歴確認
